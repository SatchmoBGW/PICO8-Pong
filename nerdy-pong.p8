...
function _update60()
	//player controls
	if btn(⬆️)
	and player.y > court_top+1 then
		player.y -= player.speed
	end
	
	if btn(⬇️)
	and player.y+player.h < court_bottom-1
	then
		player.y += player.speed
	end
	...
	and ball.y >= com.y
	and ball.y + ball.w - 1 <= com.y + com.h - 1
	then
	...
	then
		...
		if btn(⬆️) then
			if ball.dy >0 then //ball is moving down
				ball.dy =- ball.dy
		...
		if btn(⬇️) then
			if ball.dy < 0 then //ball is moving up
				ball.dy =- ball.dy
				ball.dy += ball.speedup * 2
			else
				ball.dy += ball.speedup * 2
